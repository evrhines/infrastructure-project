...
for CHANGED in "$CHANGED_FILES"; do
    echo "$CHANGED"
    if [ $(echo "$CHANGED" | grep 'terragrunt.hcl' | grep -v '.terragrunt' | grep -v 'infra/terragrunt.hcl') ]; then
...
for DELETED in "$DELETED_FILES"; do
    echo "$DELETED"
    if [ $(echo "$DELETED" | grep 'terragrunt.hcl' | grep -v '.terragrunt' | grep -v 'infra/terragrunt.hcl') ]; then
